...
object ImageFilter {
     ...
     */
    fun applyInkFilterIfEnabled(bitmap: Bitmap, preferenceStore: PreferenceStore): Bitmap {
        val isEnabled = preferenceStore.getBoolean("ink_filter", false).get()
        if (!isEnabled) return bitmap

        val bleeding = preferenceStore.getInt("ink_bleeding_intensity", 0).get()
        val bump = preferenceStore.getInt("ink_bump_intensity", 0).get()
        val original = preferenceStore.getInt("ink_original_intensity", 0).get()
                   ...
                   launch(Dispatchers.Default) {
                       ...
                       for (dy in startDy until endDy) {
                           ...
                           for (dx in -255..255) {
                               ...
                               if (lutIdx in lut.indices) lut[lutIdx] = factor
                           }
                       }
                   }
                }
