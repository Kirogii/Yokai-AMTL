...
class MangaHeaderHolder(
    ...
    fun bind(item: MangaHeaderItem) {
        ...
        with(binding.favoriteButton) {
            icon = ContextCompat.getDrawable(itemView.context, when {
        ...
        binding.filtersText.text = presenter.currentFilters()
        if (manga.isLocal()) { binding.webviewButton.isVisible = false; binding.shareButton.isVisible = false }
    ...
    private fun MaterialButton.checked(checked: Boolean) {
        if (checked) {
            ...
            backgroundTintList = ColorStateList.valueOf(ColorUtils.blendARGB(
                ...
                context.getResourceColor(R.attr.background), 0.706f))
            strokeColor = ColorStateList.valueOf(Color.TRANSPARENT)
        } else { stateListAnimator = null; resetStrokeColor()
    ...
    fun updateColors(updateAll: Boolean = true) {
        ...
        with(binding) {
            ...
            webviewButton.imageTintList = ColorStateList.valueOf(accentColor); filterButton.imageTintList = ColorStateList.valueOf(accentColor)
            trackButton.iconTint = ColorStateList.valueOf(accentColor); favoriteButton.iconTint = ColorStateList.valueOf(accentColor)
            ...
            if (updateAll) { trackButton.checked(trackButton.stateListAnimator != null); favoriteButton.checked(favoriteButton.stateListAnimator != null); setGenreTags(this, manga) } } }

    fun updateTracking() {
        binding ?: return; val presenter = adapter.delegate.mangaPresenter(); val tracked = presenter.isTracked()
        with(binding.trackButton) {
            text = itemView.context.getString(if (tracked) MR.strings.tracked else MR.strings.tracking)
            icon = ContextCompat.getDrawable(itemView.context, if (tracked) R.drawable.ic_check_24dp else R.drawable.ic_sync_24dp)
            checked(tracked) } }
